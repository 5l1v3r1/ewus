    ...
    public function __construct(Array $data)
    {
        ...
        foreach ($data as $key => $value) {
            if (in_array($key, $vars) && !is_null($value) && !in_array($key, $reject)) {
                $this->{$key} = (isset($transform[$key]) && is_callable($transform[$key]))
                    ? $transform[$key]($value)
                    : $value;
            }
        }

        if (!empty($this->faultstring)) {
            $this->isFault = true;
        }
    }
        ...
        try {
            ...
            if (isset($fault[0])) {
                ...
                $return = array(
                    ...
                    'detail_faultstring' => (string) current(
                        $xml->xpath(
                            self::NS_FAULT . 'detail/ns1:inputError/ns1:faultstring'
                        )
                    ),
                    'detail_faultcode' => (string) current(
                        $xml->xpath(self::NS_FAULT . 'detail/ns1:inputError/ns1:faultcode')
                    ),
                    'detail_faultactor' => (string) current(
                        $xml->xpath(self::NS_FAULT . 'detail/ns1:inputError/ns1:faultactor')
                    )
                );
            ...
            $system_nfz_name = $xml->xpath(
                self::NS_EXECUTE_SERVICE_RETURN
                . self::NS_STATUS_CWU_ODP
                . '/ns2:system_nfz'
            );
            ...
            $return['signature_canonicalization_method'] = (string) $signedInfo
            ...
            $return['signature_reference_digest_method'] = (string) $signedInfo
                ->Reference
                ->DigestMethod
                ->attributes()
                ->Algorithm;
